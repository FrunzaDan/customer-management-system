...
- **GUID** — customer/product primary key: a `UNIQUEIDENTIFIER` generated by the DB (`NEWSEQUENTIALID()`), returned by `POST /register`/`POST /product`, never client-supplied. See [database](database.md).
