...
- `CustomerController` (`api/Customer`) — class-level `[Authorize]`, every endpoint requires a bearer token:
  ...
  - Products & purchases (see [products-and-purchases](products-and-purchases.md)): `GET /products`, `GET /productDetails?productGuid=...`, `GET /purchases?customerGuid=...`, `POST /purchase?customerGuid=...&productGuid=...`, `POST /product`. `POST /purchase` follows the normal mutation convention (`400` bad GUID, `404` unknown customer/product, `409` deactivated customer or out of stock) and writes a `Purchased` audit entry via the new `CustomerPurchasing` class (registered alongside the other `CustomerFunctions` in `BusinessLogicDependencyInjection.cs`). `POST /product` adds a new product (`ProductRegistration`, mirrors `CustomerRegistration`'s validate-then-generate-GUID shape) but writes **no** audit entry — the audit log's `customer_guid` column is `NOT NULL`, so it has no slot for a customer-less mutation.
