...
- `CustomerController` (`/api/customer`) — class-level `[Authorize]`, every endpoint requires a bearer token:
  - `POST /register`, `GET /get?searchTerm=...`, `GET /all`, `GET /export`, `GET /audit-log?customerId=...`, `GET /audit-log/all`, `PATCH /edit`, `PATCH /deactivate?customerId=...`, `PATCH /reactivate?customerId=...`, `DELETE /delete?customerId=...`, `DELETE /audit-log/all`.
...
- **`CustomerRegistration`**: rejects missing names, missing/invalid `Email` or `PhoneNumber`, and a missing `Address` or any missing address field (`400` — every `CustomerAddress` column is `NOT NULL`, so a missing one would otherwise surface as an opaque `500`); the new ID comes back from the DB (see [database](database.md)).
- **`CustomerEditing`**: `CustomerId` is required (non-`Guid.Empty`); `Email`/`PhoneNumber`, if present in the request, are format-validated before the edit is attempted — but unlike registration, they're optional here (partial update). There's no `Status` on `UpdateCustomerRequest` at all — edit is a partial-update endpoint, not a lifecycle-transition one, so a client can't use it to bypass `DeactivateCustomer`/`ReactivateCustomer`/`DeleteCustomer`'s dedicated audit trail and stored-procedure guardrails.
...
- **`ProductRegistration`** also rejects a `Price` with more than two decimals or above `9,999,999,999.99` — what `DECIMAL(12,2)` can hold — rather than letting SQL Server silently round or overflow.
- Email/phone number **uniqueness** is enforced in `Customer_Create` (create) and `Customer_Update` (edit, excluding the row being edited itself) — `400` if either already exists on another customer. This is a DB-layer check, not duplicated in C#.
