...
- `CustomerController` (`api/Customer`) — class-level `[Authorize]`, every endpoint requires a bearer token:
  ...
  - Products & purchases (see [products-and-purchases](products-and-purchases.md)): `GET /products`, `GET /productDetails?productGuid=...`, `GET /purchases?customerGuid=...`, `POST /purchase?customerGuid=...&productGuid=...`, `POST /product`. `POST /purchase` follows the normal mutation convention (`400` bad GUID, `404` unknown customer/product, `409` deactivated customer or out of stock) and writes a `Purchased` audit entry via the new `CustomerPurchasing` class (registered alongside the other `CustomerFunctions` in `BusinessLogicDependencyInjection.cs`). `POST /product` adds a new product (`ProductRegistration`, mirrors `CustomerRegistration`: validate, then return the DB-generated GUID as `Data`) but writes **no** audit entry — the audit log's `CustomerId` column is `NOT NULL`, so it has no slot for a customer-less mutation.
  - `GET /monthlyActivity` — feeds the Charts tab's time-series charts (see [charts](charts.md)): customer registrations and product purchases, each grouped by month, from `Report_GetMonthlyActivity`. A thin passthrough like `GET /products` (no request params, no paging — both series are one row per month with any activity).
  ...
  - `DELETE /auditLog/all` permanently wipes **every** row in `CustomerAuditLog` for **every** customer (`CustomerAuditLog_DeleteAll` — a plain unconditional `DELETE`, no soft-delete). Wired to a "Clear audit log" button on the global audit log page (`global-audit-log.component.ts`'s `clearAuditLog()`), guarded client-side by a `confirm()` prompt. Carries `[Authorize(Roles = "1801")]` in addition to the class-level `[Authorize]` — a no-op today since `1801` is the only role that exists (see Known gaps below), but it stops a future second role from silently inheriting access to this destructive, untargeted action. The deletion itself is deliberately not audit-logged (no `CustomerId` to attach it to once the table is wiped).
...
- **SQL parameters are typed** (`SqlParameterExtensions`: `AddGuid`, `AddNVarChar(size)`, `AddVarChar(size)`, `AddTinyInt`, `AddSmallInt`, `AddDate`, `AddDecimal(10, 2)`), never `AddWithValue`, so each matches its proc parameter exactly — critically, the MSISDN (`@PhoneNumber`) is sent as `VARCHAR`, since an `NVARCHAR` parameter would force a conversion of the `VARCHAR` column and turn `UQ_Customer_PhoneNumber` seeks into scans. Reads use typed getters (`GetGuid`, `GetByte`, `GetInt16`, `GetDateOnly`, ...), so a schema/mapping mismatch throws instead of being silently converted.
...
- **`CustomerRegistration`**: rejects missing names, missing/invalid `Email` or `Msisdn`, and a missing `Address` or any missing address field (`400` — every `CustomerAddress` column is `NOT NULL`, so a missing one would otherwise surface as an opaque `500`); the new GUID comes back from the DB (see [database](database.md)).
...
- Email/MSISDN **uniqueness** is enforced in `Customer_Create` (create) and `Customer_Update` (edit, excluding the row being edited itself) — `400` if either already exists on another customer. This is a DB-layer check, not duplicated in C#.
...
- `Merchant_GetAuthData` fetches the hash+salt+role for a merchant ID and bumps `LastInteractionAt`; the actual comparison (`PasswordHasher.VerifyPassword`) happens in **C#, not SQL**.
...
- This is a **separate, unrelated** `global.json` from `DB/CustomerManagement/global.json`, which only pins the SQL project's SDK version (`8.0.100`) — the two don't conflict, since discovery stops at the nearest one found walking up from wherever the command runs.
