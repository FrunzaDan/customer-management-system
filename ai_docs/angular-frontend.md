...
- **Tables**: `app-card table-card` wrapping `table table-themed table-striped table-hover` (dark-green header, mint stripes/hover via Bootstrap table CSS variables). Used by the customer list and global audit log. `.table-themed` has a `min-width: 760px`, so on phones the table scrolls sideways inside its focusable `.table-responsive` region instead of crushing columns into tall wraps.
