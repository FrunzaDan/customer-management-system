...
DB_DACPAC="$DB_DIR/bin/Debug/CustomerManagement.dacpac"
UI_DIR="$ROOT_DIR/UI"
...
SQL_DATABASE="${SQL_DATABASE:-CustomerManagement}"
...
# override either.
DEFAULT_API_URL="https://localhost:7145"
if [[ -z "${API_URL:-}" ]] && [[ -f "$API_LAUNCH_SETTINGS" ]]; then
  ...
  DEFAULT_API_URL="${DEFAULT_API_URL:-https://localhost:7145}"
fi
