...
public class CustomerRegistration(IDbUtils dbUtils, ICustomerAuditLogger auditLogger)
{
    ...
    // never by the client, which has no way to supply one (CreateCustomerRequest has no CustomerId).
    public async Task<ResponseModel<Guid?>> RegisterCustomerFunction(CreateCustomerRequest request, string performedBy,
        CancellationToken cancellationToken = default)
    {
        ...
        if (response is { Status: 200, Data: { } customerId })
            await auditLogger.Log(customerId, performedBy, AuditAction.Created,
