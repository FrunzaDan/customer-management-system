...
public class CustomerRegistration(IDbUtils dbUtils, ICustomerAuditLogger auditLogger)
{
    // On success, Data is the new customer's GUID — generated by the DB (NEWSEQUENTIALID()),
    // never by the client, which has no way to supply one (CreateCustomerRequest has no Guid).
    public async Task<ResponseModel<Guid?>> RegisterCustomerFunction(CreateCustomerRequest request, string merchantId,
        CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(request.Msisdn) || MsisdnValidation.ValidateMsisdn(request.Msisdn) == false)
            return new ResponseModel<Guid?>(400, "Invalid or empty MSISDN.");
        ...
        if (request.CustomerStatus is not null and not (CustomerStatus.Active or CustomerStatus.Test))
            return new ResponseModel<Guid?>(400, "Invalid customer status.");

        var response = await dbUtils.RegisterCustomer(request, cancellationToken);
        ...
        if (response is { Status: 200, Data: { } customerGuid })
            await auditLogger.Log(customerGuid, merchantId, AuditAction.Created,
