...
public class CustomerDeletionTests
{
    ...
    [Fact]
    public async Task DeleteCustomer_DelegatesToTheDbLayerWithTheGivenCustomerId_AndLogsAnAuditEntry()
    {
        ...
        dbUtils.Setup(d => d.DeleteCustomer(customerId, It.IsAny<CancellationToken>())).ReturnsAsync(expected);
        var deletion = new CustomerDeletion(dbUtils.Object, auditLogger.Object);

        var result = await deletion.DeleteCustomer(customerId, PerformedBy, TestContext.Current.CancellationToken);

        Assert.Same(expected, result);
        dbUtils.Verify(d => d.DeleteCustomer(customerId, It.IsAny<CancellationToken>()), Times.Once);
        auditLogger.Verify(a => a.Log(customerId, PerformedBy, AuditAction.Deleted, null, It.IsAny<CancellationToken>()), Times.Once);
    }
