...
SELECT
    CONVERT(UNIQUEIDENTIFIER, v.ProductId),
    v.Name,
    v.Category,
    v.Description,
    v.Price,
...
) AS v (ProductId, Name, Category, Description, Price, InitialQuantity, QuantityOnHand, Warehouse)
WHERE NOT EXISTS (
    SELECT 1
    FROM dbo.Product AS p
    WHERE p.ProductId = CONVERT(UNIQUEIDENTIFIER, v.ProductId)
);
